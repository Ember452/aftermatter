...
**文档非铁律（元规则）**：本文件与 docs/ 下所有规范都是可修订的默认值，不是铁律。
遇到以下两种情况，**停下来打断，询问用户裁决**，不得自行默默选边：
① 文档之间存在确定性冲突（且无法按"契约唯一来源"规则自行仲裁）；
② 碰到文档未覆盖的决策点（现有规则推不出唯一答案）。
裁决后按 §二.5 写决策记录（ADR）并同步修订相关文档，再继续动手；
用户的明确指令始终高于本文件；口头决定不落文档，视为未发生。
