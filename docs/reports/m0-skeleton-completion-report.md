...
> **处置更新（2026-09-22，本报告收口后的同日追加，不回改上方原文）**：第 1 条按"让声称变成事实"
> 修复——`docs/adr/README.md` 与 `docs/specs/README.md` 各增索引表（该说法共出现在 4 处，
> 含 `AGENTS.md`、ROADMAP §1 与 `policy_checker` 的 hint 文案，因此补索引比改 4 处口径更便宜）；
> 第 2 条：`DEVELOPMENT-PLAN.md` 的 3 处 `&&` 已拆成可分别执行的命令（§1 DoD 块、§3 T0.1 验收列、
> §5 M0 首项）。两条均未起新 ADR（属措辞与可执行性修正，未改变任何既定决策）。
